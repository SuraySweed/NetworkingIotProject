...
void loop() {
  ...
  if (triggerWarning) {
    ...
    sendImageToMaster(fb); // Replace with your image sending function
  }
  oldDiff = diff; 
...
void sendImageToMaster(camera_fb_t *image) {
  ...
  // Prepare the image data for transmission
  size_t imageSize = image->len;
  uint8_t *imageData = image->buf;

  Serial.printf("imageSize = %d \n", imageSize);
  Serial.printf("String(imageData[1]) = %s \n", String(imageData[1]));

  // Create a message with the image data
  String message = "IMAGE|";
  //message += String(imageData[1]);
  //mesh.sendBroadcast(message);

  for (size_t i = 0; i < 1500; i++) {
    message += String(imageData[i]);
    if (i < 1500 - 1) {
      message += ",";
    }
    //Serial.printf("Image %u len = %u \n",i, message.length());
  }
  
  // Send the image message to the master node
  Serial.println("after fooooooooooooooooooooooooooooooooooooooooooor");
  Serial.printf("Image of len = %u\n", message.length());
  Serial.println("image is " + message);
  mesh.sendBroadcast(message);
